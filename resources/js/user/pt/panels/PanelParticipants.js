...
import { FetchPanelParticipants, FetchPanel } from '../../../components/utils/Helpers';
import { matchPath } from 'react-router';


class PanelParticipants extends React.Component {
    ...
    componentDidMount() {
        ...
        if (panelId) {
            ...
            (async () => {
                let response = await FetchPanelParticipants(panelId);
                this.setState({
                    data: response
                });
            })();
        ...
        let pageContent = <div id='user_table' className='row'>
            <div className="row w-100 mb-3 mt-3">
                <div className="col-sm-3">
                    <a className="float-left" href="/panels">&larr; Go back</a>
                </div>
                <div className="col-sm-6">
                    <h3>Panel (<b>{this.state.panel?.name}</b>) participants</h3>
                </div>
                <div className="col-sm-3">
...
if (document.getElementById('list_panel_participants')) {
    ReactDOM.render(<PanelParticipants />, document.getElementById('list_panel_participants'));
}